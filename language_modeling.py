...
questions_or_comments    = ""            # Implemented a unigram and bigram language models with add-k smoothing and perplexity evaluation
...
class NgramLanguageModel:
    ...
    def __init__(self): 
        # stops couns for single words and word pairs
        ...
        # keeps track of vocabulary and total token count
        self.vocab = set()
        self.total_tokens = 0

    def train(self, infile='samiam.train'): # reads training file, collects unigram/ bigram counts
	...
	# >>> YOUR ANSWER HERE
        with open(infile, 'r') as f:
            for line in f:  
                # splits sentence into tokens and add sentence boundary markers
                ...
                for i in range (len(tokens)):
                    ...
                    # counts each individual tokens
                    self.unigram_counts[word] += 1
                    self.vocab.add(word)
                    self.total_tokens += 1

                    # counts bigrams as previous_word_current_word
                    if i > 0:
                            prev_word = tokens[i -1]
                            bigram_key = prev_word + "_" + word
                            self.bigram_counts[bigram_key] += 1
	...
	# >>> YOUR ANSWER HERE
        ...
        # unigram prediction doesnt include <s> but </s>
        ...
        for word in tokens:
            ...
            # adds k smoothing
            # P(word) = (count(word) + k) / (total_tokens + k * vocab_size)
            prob = (word_count + self.k) / (self.total_tokens + self.k * vocab_size)

            # adds log probability instead of multiplying raw probabilities
    ...
    def predict_bigram(self, sentence):
        ...
        # bigram prediction includes both <s> and </s>
        ...
        for i in range (1, len(tokens)):
            ...
            bigram_key = prev_word + "_" + curr_word
            bigram_count = self.bigram_counts[bigram_key]
            prev_count = self.unigram_counts[prev_word]

            # add k smoothing bigram probability:
            # P(current | previous) = (count(prev, curr) + k) / (count(prev))
            prob = (bigram_count + self.k) / (prev_count + self.k * vocab_size)
        ...
        with open(test_file, 'r') as f:
            for line in f:
                ...
                if ngram_size == 'unigram':
                        total_log_prob += self.predict_unigram(sentence)
                else:
                        total_log_prob += self.predict_bigram(sentence)
                
                # count words in sentence plus the </s> token
                total_token_count += len(sentence.split()) + 1

            # perplexity = exp( - total_log_prob / N)
            avg_negative_log_prob = -total_log_prob / total_token_count
